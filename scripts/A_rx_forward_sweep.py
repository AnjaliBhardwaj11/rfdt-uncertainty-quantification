...
RX_LIST = {
    "rx0": torch.tensor([0.0, 0.0, 1.5], device=DEVICE),
    "rx1": torch.tensor([0.25, 0.0, 1.5], device=DEVICE),
    "rx2": torch.tensor([0.0, 0.25, 1.5], device=DEVICE),
}
    ...
    for th in el:
        for ph in az:
            ...
            for i in range(NX):
                for j in range(NY):
                    w[i,j] = np.exp(
                        1j*k0*d*(i*np.sin(th)*np.cos(ph)+j*np.sin(th)*np.sin(ph))
                    )
            w=w.reshape(-1)
            w/=np.linalg.norm(w)
            beams.append(w)
...
for tag, RX in RX_LIST.items():
    ...
    torch.save({"E_full":E_full,"c2":c2}, f"{tag}_mpc.pt")
    print(f"Saved {tag}_mpc.pt")
